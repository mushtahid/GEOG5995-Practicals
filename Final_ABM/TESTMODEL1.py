...
# Matplotlib for plotting and displaying environment and animals.
import matplotlib
...
modify = input("- Do you want to change the modifiable variable? Enter "\
               "'y' (lowercase) if YES (There are 11 variables that you can "\
               "modify. You will still be able to run with "\
               "default values for individual variables if you want). "\
...
if modify == correct_y:    
    ...
    n_it = input('- Enter number of iterations. The model will stop once '\
                 ...
                 'iterations (Or press enter to use default value 500): ')
    try:
    ...
    hstm = input("- Wolves have the extra ability to increase their "\
                 ...
                 "enter 1! (Or press enter to run with default "\
                 "multiplier of 2): ")
    try:
...
def update(frame_number):
    ...
    # global total_time
    # print('total_time before', total_time)
    # for i in range(no_iterations):
    #         total_time += (end-start)
    #         print('total_time after', total_time)
    ...
    # If all sheep have been eaten, stop the model!
    # See the gen function below as well!
    if len(sheep) == 0:
        carry_on = False
        print('') # Add a blank space!
        print("Stopping condition reached! All sheep have been eaten!") 
...
def gen_function():
    ...
    global carry_on
    while (a < no_iterations) & (carry_on):
        yield a
        a += 1
...
print(f"And a total process time of {total_time} seconds.")#This is not seconds!

# # Close the prinout to log file! And return stdout to origin
# sys.stdout.close()
