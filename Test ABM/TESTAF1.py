...
class Animal:
    ...
    wovles. Both animals can run closer to its own species. The sheep can run 
    away from wolves (which is not set up in the Animal class but on a
    seperate child Sheep class) and wolves can chase sheep. During movement, 
    boundary conditions are checked so that the animals stay within the limits 
    of the environment. The eating behaviours of the animals are set up in
    their own respective child classes.
    
    The store value represented by the variable store is defined in the
    constructor.    
    """
    
    def __init__(self, wolves, sheep, environment, y=None, x=None):
        """
        ...
        on random probability.
            ...
            sheep in the begining. Thus for the wolves, they fight to steal
            food from other wolves in this model.
        """
        self.wolves = wolves
        self.sheep = sheep
    ...
    def run_to_closest_animal(self, closest_animal):
        """
        Animal runs towards its closest animal and check boundary conditions.
...
class Sheep(Animal):
    ...
    def run_from_cw(self, cw):
        """
        Sheep running away from a wolf method.
        ...
        This method also ensures boundary conditions are met via callign the
...
class Wolf(Animal):
    ...
    eat method allows the wolves to eat the closest sheep. It is further 
    elaborated within itself.
    """
    ...
    # While it successfully, subsequent expansions of the code became too much 
